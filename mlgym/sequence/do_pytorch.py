...
class BinaryPredictor(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.rnn = LSTM(input_size, hidden_size, num_layers, bidirectional = (num_directions == 2))
...
for i in range(len(input_seq) - seq_len):
    window = torch.Tensor(input_seq[i:i+seq_len]).unsqueeze(1).unsqueeze(1) - 0.5
    y = torch.LongTensor([input_seq[i+seq_len]])
